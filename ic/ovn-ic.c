...
struct ic_context {
    ...
    struct ovsdb_idl_index *isbrec_port_binding_by_ts;
};
                 ...
                 const struct isbrec_availability_zone *az)
{
    ...
    INBREC_TRANSIT_SWITCH_FOR_EACH (ts, ctx->ovninb_idl) {
        ...
        const struct isbrec_port_binding *isb_pb;
        const struct isbrec_port_binding *isb_pb_key =
            isbrec_port_binding_index_init_row(ctx->isbrec_port_binding_by_ts);
        isbrec_port_binding_index_set_transit_switch(isb_pb_key, ts->name);

        ISBREC_PORT_BINDING_FOR_EACH_EQUAL (isb_pb, isb_pb_key,
                                            ctx->isbrec_port_binding_by_ts) {
        ...
        isbrec_port_binding_index_destroy_row(isb_pb_key);
...
main(int argc, char *argv[])
{
    ...
    struct ovsdb_idl_index *isbrec_port_binding_by_ts
        = ovsdb_idl_index_create1(ovnisb_idl_loop.idl,
                                  &isbrec_port_binding_col_transit_switch);
    ...
    while (!exiting) {
        if (!paused) {
            struct ic_context ctx = {
                ...
                .isbrec_port_binding_by_ts = isbrec_port_binding_by_ts,
            };
